    ...
    def test_model(self, model, dataset, angle_type="base_angle",viz=False):
        ...
        with torch.no_grad():
            for i, data in enumerate(dataloader):
                inputs, labels = data
                inputs_c = inputs.to(self.device)
                ...
                if viz== True:   
                    if k%20==0:
                        ...
                        worst_image = torch.permute(inputs[worst_idx,:3,:,:],(1,2,0))
                        best_image = torch.permute(inputs[best_idx,:3,:,:],(1,2,0))

                        worst_pred_base = np.fmod(pred[worst_idx,0],360) if pred[worst_idx,0]>0 else np.fmod(pred[worst_idx,0],360)+360
                        worst_pred_wings = np.fmod(pred[worst_idx,1],120) if pred[worst_idx,1]>0 else np.fmod(pred[worst_idx,1],120)+120
                        best_pred_base = np.fmod(pred[best_idx,0],360) if pred[best_idx,0]>0 else np.fmod(pred[best_idx,0],360)+360
                        best_pred_wings = np.fmod(pred[best_idx,1],120) if pred[best_idx,1]>0 else np.fmod(pred[best_idx,1],120)+120
