...
class AngularVectorLoss(nn.Module):
    ...
    def forward(self, pred_init, target_init, is_degrees=False):
        ...
        pred_vec = torch.stack((torch.sin(pred[:,0]), torch.cos(pred[:,0]), torch.sin(pred[:,1]), torch.cos(pred[:,1])), dim=-1)
        target_vec = torch.stack((torch.sin(target[:,0]), torch.cos(target[:,0]), torch.sin(target[:,1]), torch.cos(target[:,1])), dim=-1)
